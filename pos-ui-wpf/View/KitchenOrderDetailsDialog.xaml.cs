...
{
    public partial class KitchenOrderDetailsDialog : OptimizedDialogBase
    {
        public KitchenOrderDetailsDialog()
        {
            InitializeComponent();
            Loaded += (s, e) => ConfigureScrollViewerForTouch(OrderItemsScrollViewer);
            OrderItemsScrollViewer.ScrollChanged += OnItemsScrollChanged;
        }
        ...
        private void ConfigureScrollViewerForTouch(ScrollViewer scrollViewer)
        {
            ...
            {
                System.Diagnostics.Debug.WriteLine($"[Touch] KitchenOrderDetails scroll setup error: {ex.Message}");
            }
        }

        private void OnItemsScrollChanged(object sender, ScrollChangedEventArgs e)
        {
            if (ItemsFadeOverlay == null) return;
            var sv = sender as ScrollViewer;
            if (sv == null) return;
            bool canScrollDown = sv.VerticalOffset + sv.ViewportHeight < sv.ExtentHeight - 1;
            ItemsFadeOverlay.Opacity = canScrollDown ? 1 : 0;
        }
