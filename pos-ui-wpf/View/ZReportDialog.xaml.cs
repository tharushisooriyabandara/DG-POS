...
{
    public partial class ZReportDialog : UserControl
    {
        public ZReportDialog()
        {
            InitializeComponent();
            Loaded += (s, e) => ConfigureScrollViewerForTouch(ReportScrollViewer);
        }

        public ZReportDialog(CashDrawerSessionModel session) : this()
        {
        ...
        private void ConfigureScrollViewerForTouch(ScrollViewer scrollViewer)
        {
            ...
            {
                System.Diagnostics.Debug.WriteLine($"[Touch] ZReport scroll setup error: {ex.Message}");
            }
        }
    }
}
